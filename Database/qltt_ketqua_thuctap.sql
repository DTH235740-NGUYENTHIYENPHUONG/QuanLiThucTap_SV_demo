...
CREATE TABLE `ketqua_thuctap` (
  ...
  `DiemBaoCaoCuoiKy` decimal(4,2) DEFAULT NULL,
  `DiemTongKet` decimal(4,2) DEFAULT NULL,
  `NhanXetChung` text,
  PRIMARY KEY (`MaPhanCong`),
  CONSTRAINT `ketqua_thuctap_ibfk_1` FOREIGN KEY (`MaPhanCong`) REFERENCES `phancong_thuctap` (`ID`) ON DELETE CASCADE ON UPDATE CASCADE
